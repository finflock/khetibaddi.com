<template>
  <div>
    <div class="q-gutter-md row q-pa-md justify-center">
      <q-select  dense transition-show="scale" transition-hide="scale" v-model="model" :options="['Punjab', 'Haryana', 'Maharashtra', 'Uttar Pradesh', 'Himachal Pradesh']" label="State" style="width: 250px"/>
      <q-select  dense transition-show="scale" transition-hide="scale" v-model="model" :options="['Google', 'Facebook', 'Twitter', 'Apple', 'Oracle']" label="District" style="width: 250px"/>
      <q-select  dense transition-show="scale" transition-hide="scale" v-model="model" :options="['Apple', 'Red Chilli', 'Mangoes', 'Sunflower Seeds', 'Potato']" label="Commodity" style="width: 250px"/>
    </div>
    ...
    <div class="q-pa-md row items-start q-gutter-md">
      <q-card class="my-card" flat v-for="comodity in comodity_details" :key="comodity.id">
        ...
        <q-card-section>
          ...
          <div class="row no-wrap items-center">
            <div class="col text-overline text-capitalize text-orange-9"> {{comodity.farmer_name}} </div>
            <div class="col-auto text-grey text-caption row no-wrap items-center">
            {{comodity.farmer_location}}
          </div>
          </div>

          <div class="row no-wrap items-center">
            <q-rating size="18px" v-model="comodity.farmer_rating" :max="5" color="primary" icon-half="star_half"/>
            <span class="text-caption text-grey q-ml-sm">{{comodity.farmer_rating}} ({{comodity.farmer_reviews}})</span>
          </div>
        </q-card-section>
